...
from data_processing import tensorsFromPair
from plotting import showPlot
from data_processing import SOS_token, EOS_token
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=len(input)):
    ...
    decoder_input = torch.tensor([[SOS_token]], device=device_set())
...
def trainIters(input_lang, output_lang, pairs_input_lang, pairs_output_lang, encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    start = time.time()
    plot_losses = []
